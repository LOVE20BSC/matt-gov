...
- **初始化可验证**：部署脚本逐项读取并核对每个 `init` 后的依赖地址和参数；二次 `init` 必须回滚 `AlreadyInitialized()`，验证失败不得接入前端或发布。无权限初始化须核对实际绑定结果，被抢先绑定错误参数的版本重新部署，不能把“部署后立即初始化”视为防抢跑保证。
...
- **历史命名隔离**：迁移票据中的旧名称、旧 ABI 和旧实现事实必须明确标为历史讨论；实现只以目标仓库当前 `docs/specs/` 拆分规格和本仓库组织约束为规范依据，票据与地图只用于追溯决策来源。
...
### Phase 同步幂等
**覆盖要求**：覆盖任意地址首次调用、同一投票 Round 的跨地址重复调用、无推举时主动调用、下一 Round 再次调用，以及 Submit 首个推举触发的重复调用。

**判定标准**：每个投票 Round 最多追加一次观测并最多调整一次；重复调用返回无操作结果，不追加观测、不调整参数、不发事件，且不阻塞 Submit；下一 Round 重新允许同步。
...
### 行动公式零值边界
**覆盖要求**：覆盖 LP 时间扣减封顶、`totalEffectiveAmount` 为零，以及治理上限启用时 `totalGovVotes` 为零；覆盖链群 `totalVotes`/`totalFinalScore` 为零时不除零且行动层激励为零，并验证底层 Proposal 激励仍可独立处理。`govRatioMultiplier == 0` 时不得因治理票为零错误地清零 LP 或服务激励。
...
### 子币发射分发边界
**覆盖要求**：覆盖首个代币通过 `Launch.init(LaunchInitParams)` 一次性启动、使用 WBNB 且不带分发数据、不消耗发射次数、启动后不能重复创建首个代币、普通发射社区与 `parentTokenAddress` 一致、非零 `distributor`、普通发射的 `NoCallback`/`Callback` 两种分发模式、普通发射的 `distributorData` 可空且原样透传、`NoCallback` 下 `distributorData` 被忽略、Callback 回调失败回滚、首币固定使用 Airdrop 与 `NoCallback`、首币含名称和符号的 `TokenLaunched` 事件，以及代币列表与子币列表分页查询和符号唯一性。
...
### Proposal Target 回调
**覆盖要求**：覆盖 Proposal 创建、提案推举、提案投票三类回调；覆盖创建回调只发生一次、推举已有 Proposal 不重复创建回调、`submitterId`、`voterId`、增量票和 Target Data 透传；`submitNewProposal` 在同一笔内先创建后推举，固定先 `onProposalCreated` 再 `onProposalSubmitted`；空 Target Data 也必须触发回调；回调失败时对应外层交易整体回滚。
...
**判定标准**：
- 创建回调只调用一次
- `submitNewProposal` 同笔内先创建回调、后推举回调
- 回调失败外层交易回滚
- 参数透传无丢失
...
### Phase 与候选边界
**覆盖要求**：覆盖 Phase 边界与同步观测分账、最近 10 条观测优先及二分回溯、初始化 `ADJUST_THRESHOLD` 动态校准、首个成功推举自动同步、治理 Round 与 Phase 的一对一映射及历史不可回写；覆盖无候选人、候选票为零、平票按 `applicationId` 排序、申请版本切换、排名锁定、分割线按排名映射、开放区块向上取整和阈值区块包含判断。
...
**判定标准**：
- 动态校准超过初始化的 `ADJUST_THRESHOLD` 才生效
- 候选排序规则正确
- Phase 历史不可回写
