...
- **MemberNFT 发射次数边界**：覆盖本次铸造前剩余供应量的阈值向上取整、一次治理激励跨过多个完整阈值、整数除法余数直接忽略且不产生余数状态、每个社区达到 `maxLaunchCount = X` 后停止新增次数、同一社区源 MemberNFT 向目标 MemberNFT 部分融合转移次数、源次数扣减/目标次数增加的原子性，以及次数消耗后不能再次发射。
