...
### Phase 与候选边界
**覆盖要求**：覆盖 Phase 边界与同步观测分账、最近 10 条观测优先及二分回溯、初始化 `adjustThreshold` 动态校准、首个成功推举自动同步、治理 Round 与 Phase 的一对一映射及历史不可回写；覆盖无候选人、候选票为零、平票按 `applicationId` 排序、申请版本切换、排名锁定、分割线按排名映射、开放区块向上取整和阈值区块包含判断。
...
**判定标准**：
- 动态校准超过初始化的 `adjustThreshold` 才生效
- 候选排序规则正确
- Phase 历史不可回写
