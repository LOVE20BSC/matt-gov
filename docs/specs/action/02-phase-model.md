...
| 链群行动（四阶段） | p | p - 1 | p - 2 | p - 3 |
| 链群服务（四阶段） | p | p - 1 | p - 2，复用行动结果 | p - 3 |
...
| 3 | Round 1 LP 铸币及链群验证；Round 2 加入 |
| 4 | Round 1 链群/服务铸币；其他轮次继续流水运行 |

例：Phase 5 的加入 Round 为 4；LP 铸币 Round 为 3，链群/服务铸币 Round 为 2。

## 服务验证复用

服务 Proposal 面向整个 `actionTokenAddress` 社区的链群行动，不只绑定一个行动。服务在 Phase `N + 3` 结算 Round N，读取链群行动在 Phase `N + 2` 完成的同一 Round N 验证：

```text
proposalIds = ActionTarget.proposalIdsByExecutor(actionTokenAddress, mintRound, groupExecutor)
verified = groupActionExecutor.isRoundVerified(actionTokenAddress, actionId, mintRound)
```

仅已完成全部验证的行动计入服务权重；未验证或验证失败的跳过。不要求链群行动先完成铸币，也不在服务 Executor 内重复执行验证。聚合与分配公式见 [服务 Executor](06-service-executor.md)。

实现与组织上下文一致：LP 为三阶段，链群行动和链群服务为四阶段。
