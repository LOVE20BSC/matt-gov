...
| [服务](06-service-executor.md) | 全社区聚合、同币/直接父币、非直接关系 | 只纳入合格的同轮已验证行动，非法代币关系拒绝 |
| 服务 | 验证者比例、owner 治理上限、100% 二次分配 | 验证者按工作量直接分配；按确认方案处理 owner 超额销毁；分配不超预算、不下溢；参数来源确认后补充断言 |
