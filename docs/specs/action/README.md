...
Action 实现社群行动 Proposal 类型，包括 LP、链群行动（Group Action）和链群服务。按编号阅读；标为“待确认”的规则尚不能作为实现定案。

| 文档 | 职责 |
| --- | --- |
| [00-components.md](00-components.md) | 组件、依赖和职责边界 |
| [01-action-target.md](01-action-target.md) | 回调转发、参与登记和 forceExit |
...
| [05-group-action-executor.md](05-group-action-executor.md) | 链群索引、历史、候选和验证 |
...
建议实现顺序：ActionTarget、LP、链群行动、服务。Core 使用 `proposalId`，Executor 使用 `actionId` 业务别名，二者数值相同。
