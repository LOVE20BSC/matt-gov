...
LOVE20 是基于可验证共识行动的社群铸币协议。它通过 MemberNFT 统一身份、治理投票分配 Proposal 激励，并由不同 Executor 处理 LP、Group 和 GroupService 行动。
...
1. `Launch.init` 绑定依赖并创建首个代币，首批代币发送到 Airdrop 合约；空投份额由 LOVE20TKM 销毁活动确定。
