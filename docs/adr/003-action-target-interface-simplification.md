...
1. **事件字段不匹配**：`ActionJoined/Withdrawn/Exited` 事件包含 `amount, isExperience, providerMemberId` 字段，但 `join/exit` 函数签名没有这些参数
2. **集合读取重复**：`actionIdsByMemberId` 既提供全量数组，又提供 Count/AtIndex 分页，违反[集合设计原则](../../migration-standards.md#集合读取函数的设计原则)
3. **职责边界模糊**：ActionTarget 定位是"通用加入登记"，但事件字段包含了具体业务细节
