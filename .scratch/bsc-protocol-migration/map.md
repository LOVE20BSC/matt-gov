...
- Issue tracker：当前工作区没有 Git 远程，使用本地 Markdown tracker，文件位于本目录 `.scratch/`。
...
- 四阶段的阶段/轮次区间、动态区块校准算法、历史记录和 APY 计算基准。
- 行动执行合约的统一接口、默认行动、Join/ExtensionCenter 边界和强制退出安全模型。
