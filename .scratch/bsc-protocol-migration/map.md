...
- [四阶段与轮次模型](issues/01-phase-round-model.md) — `LOVE20Phase` 只维护无语义时间片；`action` 层统一使用 `Vote`/`Join`/`Verify`/`Mint` 四槽位 `ActionRound`，链群服务和 LP 的 `Verify` 为空操作但轮次和 `Mint` 槽位与链群行动一致。
