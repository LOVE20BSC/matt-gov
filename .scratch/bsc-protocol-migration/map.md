...
- [仓库迁移矩阵与依赖边界](issues/09-repository-migration-matrix.md) — 旧 `LOVE20Group` 并入 `core` 重命名为 `LOVE20Member`，`GroupDefaults` 不迁移；链群统一使用 `MemberNFT`，**Group Chat Delegate** 仅限 `group-chat` 内部；PancakeSwap 必须先证明 `Stake` 使用场景下的功能和数值结果正确。
