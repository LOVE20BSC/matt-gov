...
- **阈值与余数**：阈值按本次治理激励铸造前的 `maxSupply - totalSupply` 乘以初始化比例计算，并采用向上取整：`threshold = ceil((maxSupply - totalSupply) × launchRatio / 1e18)`；剩余供应量为 `0` 时不再产生阈值。一次铸造跨过多个阈值时增加对应的整数次数，未满阈值的余数继续留在 `tokenAddress + memberId` 账本。旧版 Mint 的重复铸造/重复销毁状态判断必须修复。
