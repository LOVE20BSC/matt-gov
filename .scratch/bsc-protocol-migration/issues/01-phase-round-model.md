...
- 设当前 `Phase` 编号为 `p`，`ActionTarget` 的四个槽位查询固定为 `currentMintRound() = p - 3`、`currentVerifyRound() = p - 2`、`currentJoinRound() = p - 1`、`currentVoteRound() = p`。这是同一时间点的轮次标签映射，不表示交易执行顺序；任一结果小于 `1` 时，该槽位轮次尚未开始，查询回滚而不返回 `0`。`Phase 4` 是四个槽位均有有效轮次的首个时间点。
