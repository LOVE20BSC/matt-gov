...
1. **冻结证据，不冻结旧链运行**：按合约逐项核对 `thinkium70001_public` 链上代码，记录旧仓库、源提交、合约地址、部署区块、runtime bytecode hash 和必要依赖。只把已部署合约及其必要依赖列入迁移清单；旧仓库继续保留原网络运行，不先删除或改写。
2. **创建并实现 `core`**：从清单筛选核心代码，按 BSC 规则重写 `Stake`、`Submit`、`Vote`、`Mint`、`MemberNFT`、`LOVE20Phase`、Launch 基础设施及 `LOVE20TokenFactory`，先完成单元测试和本地部署。
3. **实现业务仓库**：以 `core` 的已确认接口和 Anvil 地址为输入，建立 `launch`、`action`、`group-chat`。三者可并行开发，但各自必须先通过接口级测试；旧扩展工厂、`Verify`、`Random` 等删除项不得回流。
...
7. **正式发布与记录**：部署 `bsc56_public`，保存各仓库提交、命令、链上代码证明、地址/ABI 产物、首个代币 `distributionTarget` 和前端读写结果。

### 旧组织边界

- 在 `bsc56_public` 正式发布和验收完成前，旧组织不归档、不删除、不做会影响 Thinkium 已部署系统的重写；迁移只通过新仓库提交完成。
