...
2. **创建并实现 `core`**：从清单筛选核心代码，按 BSC 规则重写 `Stake`、`Submit`、`Vote`、`Mint`、`MemberNFT`、`Phase` 和基础子币发射能力；`TokenFactory` 职责已并入 `Launch`，先完成单元测试和本地部署。
3. **实现业务仓库**：以 `core` 的已确认接口和 Anvil 地址为输入，建立 `action`、`group-chat`；将旧 `extension-lp` 的 V2 LP 业务重写为 `action` 内的 LP 行动执行合约，并在同一代码库维护链群行动和链群服务行动执行合约。迁移阶段不创建 `launch`；V1 LP、旧扩展工厂、核心 `LOVE20Verify`、`Random` 等删除项不得回流，`GroupVerify` 仅作为 `action` 内部的可选技术拆分。
