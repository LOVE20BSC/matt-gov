...
- `action` 层所有执行合约共用 `ActionRound` 的投票、加入、验证、铸币四个业务阶段，以便前端和服务行动使用统一轮次；加入阶段包括加入、退出和行动参与状态变化，链群行动执行实际验证，链群服务和 LP 执行合约的验证阶段为空操作，不创建独立验证业务。具体执行合约仍自行判断当前 Proposal 是否满足 `canMint`，统一轮次不等于统一铸币资格。
