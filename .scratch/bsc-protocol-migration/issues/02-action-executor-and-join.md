...
- `ActionTarget.forceExit(tokenAddress, actionId, memberId)` 首版即部署，作为行动类型内部业务失效时的最后兜底；仅允许当前 `MemberNFT` 持有人调用，只清理通用参与登记并发出事件，不调用内部业务，也不承诺返还任何资产。正常加入、退出和资产取回均由行动类型内部业务负责，前端首版隐藏该入口，后续按需要开放。ActionTarget 登记是前端当前参与列表的唯一来源；Executor 的资产、参与和结算状态由 Executor 自己维护，正常登记/清除只允许关联 Executor 调用。强制退出后 ActionTarget 查询排除该记录，不回写 Executor 状态，Executor 不能利用旧状态自动恢复登记；资产恢复如有需要由 Executor 另行提供专用流程。
...
- `ActionTarget` 提供 `currentVoteRound()`、`currentJoinRound()`、`currentVerifyRound()` 和 `currentMintRound()` 四个无参数只读查询。四者都通过 `LOVE20Phase` 推导，不在 `ActionTarget` 中存储 `ActionRound` 历史；内部可共用一个按阶段计算的辅助函数，不再额外提供重复的通用 getter。
