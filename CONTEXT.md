...
- **提案回调**：通用接口只传递 `tokenAddress`、`proposalId`、当前治理轮次和不透明 KV；具体 KV 编码及初始化上下文参数由提案回调票据确认。核心不解析 KV，也不放入 `voter`、`memberId`、`votes` 等业务字段。`RewardOnly` 的 KV 必须为空；`Callback` 仅在 KV 非空时触发，目标没有合约代码或数组长度不一致则回滚。
