...
- **首发门槛**：用户可控数组和集合操作必须可分批并带游标，禁止整表扫描；批量投票、行动 KV、链群验证、奖励领取、融合和撤回在最大合法输入下必须可执行，且不牺牲原子性、安全校验和历史事件。
- **代表性交易**：覆盖 `core` 的质押/解锁/提取/融合、Proposal 创建与单批量投票、奖励准备与领取、LaunchNFT 发射、`LOVE20Phase.sync`；覆盖 `action` 的初始化、加入/撤回/退出、链群验证批次和服务激励分配；覆盖 `launch` 分配回调及 `batch-transfer` 多种规模。
- **测量方式**：同一 compiler、optimizer、profile 和等价状态下，每个用例重复 3 次取中位数，记录 `gasUsed`、calldata 大小、输入规模、最大成功批量和首次失败批量。基线读取目标网络最新区块 `gasLimit`，代表性写交易要求不超过其 80%；不把绝对 gas 数值写死进协议。
- **延后项**：低频只读 viewer、部署字节码和未影响单笔可执行性的微优化，等真实数据再处理。
