...
- **业务扩展框架层**：当前由行动框架实现，包含 `LOVE20Action`、行动数据、提案到行动的映射初始化、参与登记和应急清理入口；未来可增加与行动框架并列的其他扩展框架。行动框架不托管资产，也不定义公共验证者、链群、验证快照或行动奖励二次分配。
...
- **投票回调**：`Vote` 不解析或理解提案或扩展业务，只将 `proposalId` 和治理者附带的 KV 资料交给 `proposalTarget` 处理；行动类提案由 `LOVE20Action` 转发给对应的 `ActionExecutor`，回调失败回滚整笔投票交易。
- **投票回调触发条件**：只有 `proposalTargetMode = Callback` 且投票 KV 非空时才调用投票回调。
- **批量投票 KV**：多提案批量投票中每个 `proposalId` 可拥有独立 KV；行动扩展的私有字段由 `ActionExecutor` 自行放入并解析，通用回调不包含特定执行合约字段。
