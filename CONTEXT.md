...
- **配套仓库**：`compatibility` 专门验证外部 WBNB/WETH9 与 PancakeSwap V2 依赖的接口和实际行为；`periphery`、`script`、`love20-anvil`、`interface-test`、`interface`、`batch-transfer` 和 `docs` 是外围、部署、集成测试、前端或文档仓库，不改变核心领域关系。`compatibility` 不属于任何生产合约的运行时依赖。
