...
- **发射次数**：发射权直接记录在 `tokenAddress + memberId` 的 `launchCount` 账本中；每次治理激励实际铸造后，按本次铸造前剩余供应量计算并向上取整阈值，只按完整阈值增加整数次数，除法余数直接忽略且不保存。每个代币社区累计最多 `X` 次，达到上限后不再新增次数，发射时消耗次数。`X` 为协议初始化的每社区上限。
...
- **ActionRound**：`action` 层统一使用的四槽位轮次。当前 `Phase = p` 时，`Mint/Verify/Join/Vote` 槽位分别标记为 `p-3`、`p-2`、`p-1`、`p`；结果小于 `1` 表示该槽位轮次尚未开始，查询回滚而不返回 `0`。`Join` 表示行动参与、加入、退出和资产状态变化，不是行动类型名称。链群行动、链群服务行动和 LP 行动共享轮次，链群服务和 LP 的 `Verify` 槽位为空操作。是否满足铸币条件仍由具体执行合约判断。
- **Phase**：共享的 Phase 时间线基础设施，供核心治理层和提案扩展层使用；各层自行解释阶段和轮次。
